// "use client"
// import { useState } from "react"

// export default function projectDescriptionTwo() {
//   const [formData, setFormData] = useState({
//     nameOwner: "",
//     surnameOwner: "",
//     fathersName: "",
//     mothersName: "",
//     birthDate: "",
//     birthTown: "",
//     id: "",
//     mobile: "",
//     town: "",
//     address: "",
//     number: "",
//     postalCode: "",
//     emailOwner: "",
//     vatOwner: "",
//     projectDescription: "",
//     date: "8/2/2025",
//   })

//   const handleInputChange = (field: string, value: string) => {
//     setFormData((prev) => ({
//       ...prev,
//       [field]: value,
//     }))
//   }

//   return (
//     <div className="max-w-4xl mx-auto p-4 bg-white">
//       {/* Header with coat of arms */}
//       <div className="text-center mb-6">
//         <div className="w-16 h-16 mx-auto mb-4 bg-gray-200 rounded-full flex items-center justify-center">
//           <div className="w-12 h-12 bg-black rounded-full flex items-center justify-center">
//             <div className="w-8 h-8 bg-white rounded-sm"></div>
//           </div>
//         </div>
//         <h1 className="text-xl font-bold mb-2">ΥΠΕΥΘΥΝΗ ΔΗΛΩΣΗ</h1>
//         <p className="text-sm">(άρθρο 8 Ν.1599/1986)</p>
//       </div>

//       {/* Subtitle */}
//       <div className="text-center mb-6 text-sm">
//         <p>
//           Η ακρίβεια των στοιχείων που υποβάλλονται με αυτή τη δήλωση μπορεί να ελεγχθεί με βάση το αρχείο άλλων
//           υπηρεσιών
//         </p>
//         <p>(άρθρο 8 παρ. 4 Ν.1599/1986)</p>
//       </div>

//       {/* Form table */}
//       <div className="border border-gray-400">
//         {/* ΠΡΟΣ row */}
//         <div className="border-b border-gray-400 bg-gray-50">
//           <div className="flex">
//             <div className="w-20 p-2 border-r border-gray-400 font-bold text-sm">ΠΡΟΣ(1):</div>
//             <div className="flex-1 p-2 text-blue-600 font-bold">YDOM</div>
//           </div>
//         </div>

//         {/* Name Owner row */}
//         <div className="border-b border-gray-400">
//           <div className="flex">
//             <div className="w-32 p-2 border-r border-gray-400 text-sm">Ο-Η Όνομα</div>
//             <div className="w-40 p-1 border-r border-gray-400">
//               <input
//                 type="text"
//                 value={formData.nameOwner}
//                 onChange={(e) => handleInputChange("nameOwner", e.target.value)}
//                 className="w-full p-1 text-blue-600 font-bold border-none outline-none bg-transparent placeholder:text-blue-600 placeholder:font-normal"
//                 placeholder="Name Owner"
//               />
//             </div>
//             <div className="w-20 p-2 border-r border-gray-400 text-sm">Επώνυμο</div>
//             <div className="flex-1 p-1">
//               <input
//                 type="text"
//                 value={formData.surnameOwner}
//                 onChange={(e) => handleInputChange("surnameOwner", e.target.value)}
//                 className="w-full p-1 text-blue-600 font-bold border-none outline-none bg-transparent placeholder:text-blue-600 placeholder:font-normal"
//                 placeholder="Surname Owner"
//               />
//             </div>
//           </div>
//         </div>

//         {/* Father's name row */}
//         <div className="border-b border-gray-400">
//           <div className="flex">
//             <div className="w-32 p-2 border-r border-gray-400 text-sm">Όνομα και Επώνυμο Πατρός</div>
//             <div className="flex-1 p-1">
//               <input
//                 type="text"
//                 value={formData.fathersName}
//                 onChange={(e) => handleInputChange("fathersName", e.target.value)}
//                 className="w-full p-1 border-none outline-none bg-transparent placeholder:text-blue-600"
//                 placeholder="Fathers name and surname Owner"
//               />
//             </div>
//           </div>
//         </div>

//         {/* Mother's name row */}
//         <div className="border-b border-gray-400">
//           <div className="flex">
//             <div className="w-32 p-2 border-r border-gray-400 text-sm">Όνομα και Επώνυμο Μητρός</div>
//             <div className="flex-1 p-1">
//               <input
//                 type="text"
//                 value={formData.mothersName}
//                 onChange={(e) => handleInputChange("mothersName", e.target.value)}
//                 className="w-full p-1 border-none outline-none bg-transparent placeholder:text-blue-600"
//                 placeholder="Mothers name and surname Owner"
//               />
//             </div>
//           </div>
//         </div>

//         {/* Birth date row */}
//         <div className="border-b border-gray-400">
//           <div className="flex">
//             <div className="w-32 p-2 border-r border-gray-400 text-sm">Ημερομηνία γέννησης(2):</div>
//             <div className="flex-1 p-1">
//               <input
//                 type="date"
//                 value={formData.birthDate}
//                 onChange={(e) => handleInputChange("birthDate", e.target.value)}
//                 className="w-full p-1 border-none outline-none bg-transparent placeholder:text-blue-600"
//               />
//             </div>
//           </div>
//         </div>

//         {/* Birth town row */}
//         <div className="border-b border-gray-400">
//           <div className="flex">
//             <div className="w-32 p-2 border-r border-gray-400 text-sm">Τόπος Γέννησης</div>
//             <div className="flex-1 p-1">
//               <input
//                 type="text"
//                 value={formData.birthTown}
//                 onChange={(e) => handleInputChange("birthTown", e.target.value)}
//                 className="w-full p-1 text-blue-600 border-none outline-none bg-transparent placeholder:text-blue-600"
//                 placeholder="Born Town owner"
//               />
//             </div>
//           </div>
//         </div>

//         {/* ID and mobile row */}
//         <div className="border-b border-gray-400">
//           <div className="flex">
//             <div className="w-32 p-2 border-r border-gray-400 text-sm">Αριθμός Δελτίου Ταυτότητας</div>
//             <div className="w-20 p-1 border-r border-gray-400">
//               <input
//                 type="text"
//                 value={formData.id}
//                 onChange={(e) => handleInputChange("id", e.target.value)}
//                 className="w-full p-1 text-blue-600 border-none outline-none bg-transparent placeholder:text-blue-600"
//                 placeholder="ID"
//               />
//             </div>
//             <div className="w-16 p-2 border-r border-gray-400 text-sm">Τηλ.:</div>
//             <div className="flex-1 p-1">
//               <input
//                 type="tel"
//                 value={formData.mobile}
//                 onChange={(e) => handleInputChange("mobile", e.target.value)}
//                 className="w-full p-1 text-blue-600 border-none outline-none bg-transparent placeholder:text-blue-600"
//                 placeholder="mobile"
//               />
//             </div>
//           </div>
//         </div>

//         {/* Address row */}
//         <div className="border-b border-gray-400">
//           <div className="flex">
//             <div className="w-32 p-2 border-r border-gray-400 text-sm">Τόπος κατοικίας</div>
//             <div className="w-20 p-1 border-r border-gray-400">
//               <input
//                 type="text"
//                 value={formData.town}
//                 onChange={(e) => handleInputChange("town", e.target.value)}
//                 className="w-full p-1 text-blue-600 border-none outline-none bg-transparent placeholder:text-blue-600"
//                 placeholder="Town"
//               />
//             </div>
//             <div className="w-16 p-2 border-r border-gray-400 text-sm">Οδός</div>
//             <div className="w-24 p-1 border-r border-gray-400">
//               <input
//                 type="text"
//                 value={formData.address}
//                 onChange={(e) => handleInputChange("address", e.target.value)}
//                 className="w-full p-1 text-blue-600 border-none outline-none bg-transparent placeholder:text-blue-600"
//                 placeholder="Address"
//               />
//             </div>
//             <div className="w-16 p-2 border-r border-gray-400 text-sm">Αριθ</div>
//             <div className="w-20 p-1 border-r border-gray-400">
//               <input
//                 type="text"
//                 value={formData.number}
//                 onChange={(e) => handleInputChange("number", e.target.value)}
//                 className="w-full p-1 text-blue-600 border-none outline-none bg-transparent placeholder:text-blue-600"
//                 placeholder="Number"
//               />
//             </div>
//             <div className="w-12 p-2 border-r border-gray-400 text-sm">ΤΚ</div>
//             <div className="flex-1 p-1">
//               <input
//                 type="text"
//                 value={formData.postalCode}
//                 onChange={(e) => handleInputChange("postalCode", e.target.value)}
//                 className="w-full p-1 text-blue-600 border-none outline-none bg-transparent placeholder:text-blue-600"
//                 placeholder="postal code"
//               />
//             </div>
//           </div>
//         </div>

//         {/* Contact details row */}
//         <div className="border-b border-gray-400">
//           <div className="flex">
//             <div className="w-32 p-2 border-r border-gray-400 text-sm">Αρ. Τηλεομοιότυπου (Fax):</div>
//             <div className="flex-1 p-2">
//               <div className="text-sm">
//                 <div>Δ/νση</div>
//                 <div>Ηλεκτρ.</div>
//                 <div>Ταχυδρομ</div>
//                 <div>ίου (Email):</div>
//               </div>
//             </div>
//             <div className="w-32 p-1">
//               <input
//                 type="email"
//                 value={formData.emailOwner}
//                 onChange={(e) => handleInputChange("emailOwner", e.target.value)}
//                 className="w-full p-1 text-blue-600 border-none outline-none bg-transparent placeholder:text-blue-600"
//                 placeholder="email owner"
//               />
//             </div>
//           </div>
//         </div>

//         {/* VAT row */}
//         <div className="border-b border-gray-400">
//           <div className="flex">
//             <div className="w-32 p-2 border-r border-gray-400 text-sm">Α.Φ.Μ.:</div>
//             <div className="flex-1 p-1">
//               <input
//                 type="text"
//                 value={formData.vatOwner}
//                 onChange={(e) => handleInputChange("vatOwner", e.target.value)}
//                 className="w-full p-1 text-blue-600 border-none outline-none bg-transparent placeholder:text-blue-600"
//                 placeholder="VAT owner"
//               />
//             </div>
//             <div className="w-32 p-2 border-l border-gray-400 text-sm">Δ.Ο.Υ.:</div>
//           </div>
//         </div>

//         {/* Declaration text */}
//         <div className="p-4 text-sm">
//           <p className="mb-4">
//             Με στοιχεία μου ευθύνη και γνωρίζοντας τις κυρώσεις(3), που προβλέπονται από τις διατάξεις της παρ. 6 του
//             άρθρου 22 του Ν.1599/1986, δηλώνω ότι:
//           </p>

//           <p className="mb-4">
//             ως Κύριος/διοικητής του ακινήτου που βρίσκεται επί της οδού (Address, Number, Town, Postal code),συναινώ
//             ρητά και ανεπιφύλακτα στην έκδοση Έγκρισης Εργασιών Μικρής Κλίμακας για την εκτέλεση των εργασιών,
//           </p>

//           <p className="mb-4">για το έργο με τίτλο :</p>
//           <textarea
//             value={formData.projectDescription}
//             onChange={(e) => handleInputChange("projectDescription", e.target.value)}
//             className="w-full p-2 text-blue-600 font-bold border border-gray-300 rounded resize-none placeholder:text-blue-600 placeholder:font-normal"
//             placeholder="PROJECT DESCRIPTION"
//             rows={3}
//           />
//         </div>

//         {/* Additional disclaimer text */}
//         <div className="p-4 text-sm bg-blue-50 border-t border-gray-400">
//           <p className="text-blue-600">
//             Η παρούσα δήλωση παρέχεται αποκλειστικά για την κάλυψη των απαιτήσεων του άρθρου 29 του Ν.4495/2017 και των
//             σχετικών κανονιστικών πράξεων.
//           </p>
//         </div>

//         {/* Signature section */}
//         <div className="border-t border-gray-400">
//           <div className="flex justify-end p-4">
//             <div className="text-right space-y-2">
//               <div className="flex items-center gap-4">
//                 <span className="text-sm">Ημερομηνία :</span>
//                 <input
//                   type="date"
//                   value={formData.date}
//                   onChange={(e) => handleInputChange("date", e.target.value)}
//                   className="text-sm font-medium border-none outline-none bg-transparent placeholder:text-blue-600"
//                 />
//               </div>
//               <div className="text-sm mt-8">
//                 <div>( Υπογραφή )</div>
//                 <div className="mt-4">Ο/Η Δηλώνουσα</div>
//               </div>
//             </div>
//           </div>
//         </div>
//       </div>
//     </div>
//   )
// }




export default function ProjectDescriptionTwo() {
  return (
    <div className="max-w-4xl mx-auto p-4 bg-white">
      {/* Header with coat of arms */}
      <div className="text-center mb-6">
        <div className="w-16 h-16 mx-auto mb-4 bg-gray-200 rounded-full flex items-center justify-center">
          <div className="w-12 h-12 bg-black rounded-full flex items-center justify-center">
            <div className="w-8 h-8 bg-white rounded-sm"></div>
          </div>
        </div>
        <h1 className="text-xl font-bold mb-2">ΥΠΕΥΘΥΝΗ ΔΗΛΩΣΗ</h1>
        <p className="text-sm">(άρθρο 8 Ν.1599/1986)</p>
      </div>

      {/* Subtitle */}
      <div className="text-center mb-6 text-sm">
        <p>
          Η ακρίβεια των στοιχείων που υποβάλλονται με αυτή τη δήλωση μπορεί να ελεγχθεί με βάση το αρχείο άλλων
          υπηρεσιών
        </p>
        <p>(άρθρο 8 παρ. 4 Ν.1599/1986)</p>
      </div>

      {/* Form table */}
      <div className="border border-gray-400">
        {/* ΠΡΟΣ row */}
        <div className="border-b border-gray-400 bg-gray-50">
          <div className="flex">
            <div className="w-20 p-2 border-r border-gray-400 font-bold text-sm">ΠΡΟΣ(1):</div>
            <div className="flex-1 p-2 text-blue-600 font-bold">YDOM</div>
          </div>
        </div>

        {/* Name Owner row */}
        <div className="border-b border-gray-400">
          <div className="flex">
            <div className="w-32 p-2 border-r border-gray-400 text-sm">Ο-Η Όνομα</div>
            <div className="w-40 p-2 border-r border-gray-400 text-blue-600 font-bold">Name Owner</div>
            <div className="w-20 p-2 border-r border-gray-400 text-sm">Επώνυμο</div>
            <div className="flex-1 p-2 text-blue-600 font-bold">Surname Owner</div>
          </div>
        </div>

        {/* Father's name row */}
        <div className="border-b border-gray-400">
          <div className="flex">
            <div className="w-32 p-2 border-r border-gray-400 text-sm">Όνομα και Επώνυμο Πατρός</div>
            <div className="flex-1 p-2">Fathers name and surname Owner</div>
          </div>
        </div>

        {/* Mother's name row */}
        <div className="border-b border-gray-400">
          <div className="flex">
            <div className="w-32 p-2 border-r border-gray-400 text-sm">Όνομα και Επώνυμο Μητρός</div>
            <div className="flex-1 p-2">Mothers name and surname Owner</div>
          </div>
        </div>

        {/* Birth date row */}
        <div className="border-b border-gray-400">
          <div className="flex">
            <div className="w-32 p-2 border-r border-gray-400 text-sm">Ημερομηνία γέννησης(2):</div>
            <div className="flex-1 p-2">Born date Owner</div>
          </div>
        </div>

        {/* Birth town row */}
        <div className="border-b border-gray-400">
          <div className="flex">
            <div className="w-32 p-2 border-r border-gray-400 text-sm">Τόπος Γέννησης</div>
            <div className="flex-1 p-2 text-blue-600">Born Town owner</div>
          </div>
        </div>

        {/* ID and mobile row */}
        <div className="border-b border-gray-400">
          <div className="flex">
            <div className="w-32 p-2 border-r border-gray-400 text-sm">Αριθμός Δελτίου Ταυτότητας</div>
            <div className="w-20 p-2 border-r border-gray-400 text-blue-600">ID</div>
            <div className="w-16 p-2 border-r border-gray-400 text-sm">Τηλ.:</div>
            <div className="flex-1 p-2 text-blue-600">mobile</div>
          </div>
        </div>

        {/* Address row */}
        <div className="border-b border-gray-400">
          <div className="flex">
            <div className="w-32 p-2 border-r border-gray-400 text-sm">Τόπος κατοικίας</div>
            <div className="w-20 p-2 border-r border-gray-400 text-blue-600">Town</div>
            <div className="w-16 p-2 border-r border-gray-400 text-sm">Οδός</div>
            <div className="w-24 p-2 border-r border-gray-400 text-blue-600">Address</div>
            <div className="w-16 p-2 border-r border-gray-400 text-sm">Αριθ</div>
            <div className="w-20 p-2 border-r border-gray-400 text-blue-600">Number</div>
            <div className="w-12 p-2 border-r border-gray-400 text-sm">ΤΚ</div>
            <div className="flex-1 p-2 text-blue-600">postal code</div>
          </div>
        </div>

        {/* Contact details row */}
        <div className="border-b border-gray-400">
          <div className="flex">
            <div className="w-32 p-2 border-r border-gray-400 text-sm">Αρ. Τηλεομοιότυπου (Fax):</div>
            <div className="flex-1 p-2">
              <div className="text-sm">
                <div>Δ/νση</div>
                <div>Ηλεκτρ.</div>
                <div>Ταχυδρομ</div>
                <div>ίου (Email):</div>
              </div>
            </div>
            <div className="w-32 p-2 text-blue-600">email owner</div>
          </div>
        </div>

        {/* VAT row */}
        <div className="border-b border-gray-400">
          <div className="flex">
            <div className="w-32 p-2 border-r border-gray-400 text-sm">Α.Φ.Μ.:</div>
            <div className="flex-1 p-2 text-blue-600">VAT owner</div>
            <div className="w-32 p-2 border-l border-gray-400 text-sm">Δ.Ο.Υ.:</div>
          </div>
        </div>

        {/* Declaration text */}
        <div className="p-4 text-sm">
          <p className="mb-4">
            Με στοιχεία μου ευθύνη και γνωρίζοντας τις κυρώσεις(3), που προβλέπονται από τις διατάξεις της παρ. 6 του
            άρθρου 22 του Ν.1599/1986, δηλώνω ότι:
          </p>

          <p className="mb-4">
            ως Κύριος/διοικητής του ακινήτου που βρίσκεται επί της οδού (Address, Number, Town, Postal code),συναινώ
            ρητά και ανεπιφύλακτα στην έκδοση Έγκρισης Εργασιών Μικρής Κλίμακας για την εκτέλεση των εργασιών,
          </p>

          <p className="mb-4">για το έργο με τίτλο :</p>
          <p className="text-blue-600 font-bold">PROJECT DESCRIPTION</p>
        </div>

        {/* Additional disclaimer text */}
        <div className="p-4 text-sm bg-blue-50 border-t border-gray-400">
          <p className="text-blue-600">
            Η παρούσα δήλωση παρέχεται αποκλειστικά για την κάλυψη των απαιτήσεων του άρθρου 29 του Ν.4495/2017 και των
            σχετικών κανονιστικών πράξεων.
          </p>
        </div>

        {/* Signature section */}
        <div className="border-t border-gray-400">
          <div className="flex justify-end p-4">
            <div className="text-right space-y-2">
              <div className="flex items-center gap-4">
                <span className="text-sm">Ημερομηνία :</span>
                <span className="text-sm font-medium">8/2/2025</span>
              </div>
              <div className="text-sm mt-8">
                <div>( Υπογραφή )</div>
                <div className="mt-4">Ο/Η Δηλώνουσα</div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}















